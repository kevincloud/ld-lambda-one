...
resource "aws_lambda_function_url" "lambda_incrementnone_url" {
  function_name      = aws_lambda_function.lambda_incrementnone.arn
  ...
  depends_on = [aws_lambda_function.lambda_incrementnone]
}
